...
def NodeValid(node):            #Function checks for the existance for the solution
    x=node.ravel()
    x=x.tolist()
...
def nodecode(J):                                 #Function converts 3X3 node to an integer code specific to a particular node
	chain = list(itertools.chain(*J))
	G = ''.join(map(str, chain))
	return G
def BlankTileLocation(Current_Node):            #Function checks the location of '0' that is null space in 3X3 matrix
	cp=np.array([1,2])
	for k in range(0,3):
		for l in range(0,3):
...
def MoveRight(Current_Node):                    #Function to move the blank space right
	c=np.array([0,1])
...
def MoveLeft(Current_Node):                   #Function to move the blank space left
	c=np.array([0,-1])
	cl=BlankTileLocation(Current_Node)
	if(cl[1]==0):
		flag=1
...
def MoveUp(Current_Node):                       #Function to move the blank space up
	c=np.array([-1,0])
	cl=BlankTileLocation(Current_Node)
	if(cl[0]==0):
		flag=1
...
def MoveDown(Current_Node):                  #Function to move the blank space down
	c=np.array([1,0])
	cl=BlankTileLocation(Current_Node)
	if(cl[0]==2):
		flag=1
...
def InputNode():            #Function to take input and generate an nd array - used to generate user entered Initial Node
    ...
    a = [int(x) for x in input("Enter Row 1 - ").split()]
    b = [int(x) for x in input("Enter Row 2 - ").split()]
    c = [int(x) for x in input("Enter Row 3 - ").split()]
...
#------------------ Same is done for the rest of the operations i.e. Down , Left and Right ------------------
    ...
    # node = open("nodePath.txt","w")
    # for j in nodecodeset:
    #     #nodeDisplay()
    #     data = nodecode(j)
    #     for i in range(0, len(solution)):
    #         nodePath.write(str(int(nodeDisplay[i])) + " ")
    #     nodePath.write("\n")
    # nodePath.close()
    #
    # nodes_info = open('NodesInfo1.txt','w')
    # for i in range (0, len(solution[fw])-1):
    #     nodes_info.writelines(str(solution[i]) + " " + str (parentindex[i]) + "\n")
    #     nodes_info.close()
